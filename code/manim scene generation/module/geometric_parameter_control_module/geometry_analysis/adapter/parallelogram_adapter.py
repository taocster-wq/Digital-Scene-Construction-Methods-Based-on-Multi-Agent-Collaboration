...
class ParallelogramAdapter(GeometryAdapter):
    """平行四边形适配器"""
    ...
    def draw(self, ax):
        ...
        parallelogram = Polygon([vA, vB, vC, vD], closed=True, fill=False, edgecolor='pink', linewidth=2)
        ax.add_patch(parallelogram)
        ax.plot(vA[0], vA[1], 'o', color='pink', markersize=8)
        ax.plot(vB[0], vB[1], 'o', color='pink', markersize=8)
        ax.plot(vC[0], vC[1], 'o', color='pink', markersize=8)
        ax.plot(vD[0], vD[1], 'o', color='pink', markersize=8)
