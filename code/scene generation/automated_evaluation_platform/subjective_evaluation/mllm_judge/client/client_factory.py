...
try:
    ...
    if project_root is None:
        raise RuntimeError("Unable to locate the project root. Please run the program from the project root directory.")

    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))

    from automated_evaluation_platform.subjective_evaluation.mllm_judge import client
    from automated_evaluation_platform.subjective_evaluation.mllm_judge.client import default_client
...
class ClientFactory(metaclass=SingletonMeta):
    ...
    def _load_json_config(path: Optional[str]) -> dict:
        ...
        try:
            ...
            print(f"[ClientFactory] Failed to read config: {e}")
            return {}
