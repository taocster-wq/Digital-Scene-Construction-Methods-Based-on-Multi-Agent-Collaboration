    ...
    async def execute_task(
        ...
        if not image_list:
            ...
            for idx, url in enumerate(image_list):
                content.append({"type": "text", "text": f"Animation frame {idx}:"})
                content.append(
                    {
                        "type": "image_url",
                        "image_url": {"url": url},
        ...
        if not image_list:
            ...
            for idx, item in enumerate(image_list, start=1):
                content.append(
                    {
                        "type": "text",
                        "text": f"Frame {idx}: original image, bounding box image, and keypoint image",
                    }
                )
                    ...
                    for key in ["original", "bbox", "points"]:
                        ...
                        if url:
                            ...
                            content.append(
                                {
                                    "type": "text",
                                    "text": f"[WARN] Frame {idx} is missing the {key} image or url",
                                }
                            )
