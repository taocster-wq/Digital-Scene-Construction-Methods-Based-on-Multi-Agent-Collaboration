...
def start_manim_command(quality, scene_code_file_path, class_name, err_message):
    ...
    envlog_path = project_root / "error.env.log"
    ...
    env["PYTHONPATH"] = (
        str(project_root)
        + (os.pathsep + old_python_path if old_python_path else "")
    )
    ...
    try:
        with open(envlog_path, "w", encoding="utf-8", errors="replace") as f:
            ...
            path_head = (path_value[:300] + "...") if path_value else ""

            f.write(f"PATH(head)={path_head}\n")
